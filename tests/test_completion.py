...
from mcv_cli.completion import complete_course_group, completion_items
...
def test_completion_uses_readable_courses_and_canonical_refs(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("mcv_cli.completion.active_cache", lambda: cache)
    ...
    assert [item.value for item in complete_course_group(
