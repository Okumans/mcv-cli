...
from mcv_cli.errors import StorageError
...
def test_keyring_is_preferred(monkeypatch, tmp_path, profile) -> None:
    ...
    monkeypatch.setattr("mcv_cli.storage.keyring", fake_keyring)
    store = __import__("mcv_cli.storage", fromlist=["CredentialStore"]).CredentialStore(
        file_path=tmp_path / "credentials.enc",
        prefer_keyring=True,
    )
