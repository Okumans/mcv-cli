...
from mcv_cli.auth import AuthManager, build_public_authorization_url, normalize_provider
from mcv_cli.constants import (
    BASE_URL,
    CHULA_LOGIN_URL,
    PLATFORM_LOGIN_URL,
    PUBLIC_AUTHORIZATION_URL,
)
from mcv_cli.errors import AuthenticationError, ConfigurationError
from mcv_cli.models import AuthProvider
...
def test_public_authorization_url_uses_the_site_client_and_provider_hint() -> None:
    platform_query = parse_qs(
        urlparse(build_public_authorization_url(AuthProvider.PLATFORM)).query
    )
...
def test_mcv_provider_is_an_alias_for_platform() -> None:
    assert normalize_provider(AuthProvider.MCV) is AuthProvider.PLATFORM
