...
from .errors import AuthenticationError, AuthenticationRequired, ConfigurationError
...
def normalize_provider(provider: AuthProvider) -> AuthProvider:
    """Treat the historical ``mcv`` provider name as platform login."""

    return AuthProvider.PLATFORM if provider is AuthProvider.MCV else provider


def build_public_authorization_url(provider: AuthProvider) -> str:
    """Build the public MyCourseVille login link used to initialize a session.

    This is the link currently exposed by MyCourseVille's own landing page. It
    identifies MyCourseVille's web client and is not an application credential
    belonging to mcv.
    """
    ...
    elif selected is AuthProvider.GOOGLE:
...
class _LoginPageParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag.lower() == "form" and self._form_action is None:
            ...
            if "/api/oauth/authorize" in href:
                self.oauth_links.append(href)
...
class AuthManager:
        ...
        with httpx.Client(
            ...
            if response.status_code >= 400:
                raise _authentication_http_error(
                    "MyCourseVille rejected the login request",
                    response,
                )
            ...
            if response.status_code >= 400:
                raise _authentication_http_error(
                    "MyCourseVille rejected the login redirect",
                    response,
                )
    ...
    def logout(self) -> None:
        ...
        if profile is None:
            return
        self.store.save(StoredProfile(provider=normalize_provider(profile.provider)))
    ...
    def _load_login_form(self, client: httpx.Client, provider: AuthProvider) -> LoginForm:
        ...
        if response.status_code >= 400:
            raise _authentication_http_error(
                "MyCourseVille did not open its login page",
                response,
            )
    ...
    def _verify_session(self, client: httpx.Client) -> None:
        ...
        if response.status_code in _REDIRECT_STATUSES or response.status_code == 401:
            raise _authentication_http_error(
                "MyCourseVille rejected the credentials or did not establish a session",
                response,
            )
        if response.status_code >= 400:
            raise _authentication_http_error(
                "MyCourseVille could not verify the login",
                response,
            )
    ...
    def _find_session_redirect(self, response: httpx.Response) -> str | None:
        ...
        for href in parser.oauth_links:
            target = self._safe_internal_url(response.url, href)
            return target
        return None
    ...
    def _collect_cookies(client: httpx.Client) -> dict[str, str]:
        ...
        for cookie in client.cookies.jar:
            ...
            if (
                (
                    not domain
                    or domain == "mycourseville.com"
                    or domain.endswith(".mycourseville.com")
                )
